...
//                    inputStream = Files.newInputStream(Paths.get(userDir + "/test-classes/configuration-profiles/" + System.getenv("PROPERTIES_FILES")));
                    inputStream = Files.newInputStream(Paths.get(userDir + "/test-classes/configuration-profiles/ios.azure.properties"));
                    properties.load(inputStream);
                } else {
                    inputStream = Files.newInputStream(Paths.get(userDir + "/src/test/resources/configuration-profiles/" + propertiesFileName));
                    properties.load(inputStream);
    ...
    public static boolean getBooleanProperty(String propertyKey) {
        ...
        boolean propertyValueFromSystem = Boolean.parseBoolean(System.getProperty(propertyKey, propertyValueFromFile));
        LoggingManager.logDebug(PropertyUtils.class, propertyKey + " = " + propertyValueFromSystem);
        return propertyValueFromSystem;
    }
    ...
    public static String getPropertiesFiles() {
        return System.getProperty(Constants.PROPERTY_PROPERTIES_FILES, Constants.DEFAULT_AZURE_PROPERTIES_FILES);
    }
